...
def bboxkpss2result(bboxes, kpss, labels, num_classes):
    """Convert detection results to a list of numpy arrays.
    ...
    results = []
    if bboxes.shape[0] == 0:
        results.append([np.zeros((0, 5), dtype=np.float32) for i in range(num_classes)])
        results.append([np.zeros((0, 9), dtype=np.float32) for i in range(num_classes)])
    else:
        if isinstance(bboxes, torch.Tensor):
            bboxes = bboxes.detach().cpu().numpy()
            kpss = kpss.detach().cpu().numpy()
            labels = labels.detach().cpu().numpy()
        results = []
        results.append([bboxes[labels == i, :] for i in range(num_classes)])
        results.append([kpss[labels == i, :] for i in range(num_classes)])
        return results
