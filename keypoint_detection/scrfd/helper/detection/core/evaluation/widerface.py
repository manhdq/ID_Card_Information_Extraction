...
def norm_score(pred):
    """ norm score
    pred {key: [[x1,y1,x2,y2,x3,y3,x4,y4,s]]}
    """
...
def kptss_overlap(a_s, b):
    ...
    for a in a_s:
        ...
        poly_intersect = poly_a.intersection(poly_b).area
        poly_union = poly_a.union(poly_b).area
        kiou = poly_intersect / poly_union
        o.append(kiou)
    o = np.array(o)
    return o


def image_kpts_eval(pred, gt, iou_thresh, mpp):
    """ single image evaluation
    pred: Nx5
    gt: Nx4
    ignore:
    ...
    recall_list = np.zeros(_gt.shape[0])
    proposal_list = np.ones(_pred.shape[0])
    kious = np.zeros(_gt.shape[0])
    # gt_overlap_list = kptss_overlap(_gt, _pred[0])
    # print(len(_pred))
    # print(_gt)
    # exit(0)

    gt_overlap_list = mpp.starmap(kptss_overlap, zip([_gt]*_pred.shape[0],[_pred[h] for h in range(_pred.shape[0])]))
    ...
    for h in range(_pred.shape[0]):
        ...
        #print ('overlap', gt_overlap)
        max_overlap, max_idx = gt_overlap.max(), gt_overlap.argmax()
        kious[max_idx] = max(max_overlap, kious[max_idx])
    ...
    return pred_recall, proposal_list, kious


def img_kpts_pr_info(thresh_num, pred_info, proposal_list, pred_recall):
    pr_info = np.zeros((thresh_num, 2)).astype('float')
    fp = np.zeros((pred_info.shape[0],), dtype=np.int)
    last_info = [-1, -1]
    for t in range(thresh_num):
...
def wider_kpts_evaluation(preds, gts, iou_thresh=0.5, debug=False):
    #pred = get_preds(pred)
    img_list = preds.keys()
    kious = []

    preds = norm_score(preds)
    thresh_num = 1000
    #thresh_num = 2000
    ...
    for i, img_name in enumerate(img_list):
        ...
        pred_list = preds[img_name][0]
        gt_list = gts[img_name][0]
        count_objs += len(gt_list)

        if len(pred_list) == 0 or len(gt_list) == 0:
            continue

        pred_recall, proposal_list, kiou_list = image_kpts_eval(pred_list, gt_list, iou_th, mpp)
        kious.append(kiou_list)

        _img_pr_info, fp = img_kpts_pr_info(thresh_num, pred_list, proposal_list, pred_recall)

        pr_curve += _img_pr_info
        
    #print ('pr_curve', pr_curve, count_face)
    pr_curve = dataset_pr_info(thresh_num, pr_curve, count_objs)
    ...
    print('cost %.4f seconds, kiou thresh: %.2f, ap: %.5f'%((tb-ta).total_seconds(), iou_thresh, ap))

    kious = np.concatenate(kious)
    return ap, kious
