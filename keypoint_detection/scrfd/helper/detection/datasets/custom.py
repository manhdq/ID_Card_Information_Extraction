...
from ...general.fileio import load, list_from_file
from ..core import eval_map, eval_recalls
from .pipelines import Compose
from .pipelines import MultiScaleFlipAug
...
class CustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        ### TODO: modified code for load seg, load proposals
    ...
    def pre_pipeline(self, results):
        ...
        prefix_idx_valid = None
        for prefix_idx in range(len(self.img_prefix)):
            cur_img_prefix = self.img_prefix[prefix_idx]
            if os.path.isfile(os.path.join(cur_img_prefix, img_filename)):
                prefix_idx_valid = prefix_idx
                break
        
        assert prefix_idx_valid is not None
        img_prefix_valid = self.img_prefix[prefix_idx_valid]
        seg_prefix_valid = None
        proposal_file_valid = None
        if self.seg_prefix is not None:
            seg_prefix_valid = self.seg_prefix[prefix_idx_valid]
        if self.proposal_file is not None:
            proposal_file_valid = self.proposal_file[prefix_idx_valid]

        results['img_prefix'] = img_prefix_valid
        results['seg_prefix'] = seg_prefix_valid
        results['proposal_file'] = proposal_file_valid
        results['bbox_fields'] = []
        results['mask_fields'] = []
        results['seg_fields'] = []
    ...
    def prepare_multiple_train_imgs(self, idx, num_samples):
        ...
        selected_indices = np.random.choice(total_indices, num, False)
