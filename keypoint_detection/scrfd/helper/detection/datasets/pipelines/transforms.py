...
from ....general.utils import is_list_of, is_str
from ....general.image import imrescale, imresize, imflip, impad, impad_to_multiple, \
                            imnormalize, bgr2hsv, hsv2bgr
...
def drawKpts(img, kptss):
    import cv2

    assert kptss.shape[-1] == 3
    assert kptss.shape[1] == 4
    ...
    for kpts in kptss:
        for idx, kpt in enumerate(kpts):
            ...
            kpt = kpt.astype(int)
            kpt = tuple(kpt[:2])
            img = cv2.circle(img, kpt, 20, colors[idx], 2)
...
def drawBoxes(img, bboxes):
    ...
    for bbox in bboxes:
        img = np.array(img).astype(np.uint8)
        s_p = tuple(bbox[:2].astype(int))
        e_p = tuple(bbox[-2:].astype(int))
        img = cv2.rectangle(img, s_p, e_p, (123, 123, 0), 2)
        ...
        if results['flip']:
            ...
            for key in results.get('keypoints_fields', []):
                results[key] = self.keypoints_flip(results[key],
                                              results['img_shape'],
                                              results['flip_direction'])
            # img = drawKpts(results['img'], results['gt_keypointss'])
            # import cv2
            # if results['flip_direction'] is None:
            #     cv2.imwrite('test.jpg', img)
            # elif results['flip_direction'] =='horizontal':
            #     cv2.imwrite('test_hori.jpg', img)
            # elif results['flip_direction'] =='vertical':
            #     cv2.imwrite('test_vert.jpg', img)
            # elif results['flip_direction'] =='diagonal':
            #     cv2.imwrite('test_diag.jpg', img)
...
class Albu(object):
    ...
    def __init__(self,
                 ...
                 refine_bbox_from_keypoint=False,
                 keymap=None,
        ...
        self.refine_bbox_from_keypoint = refine_bbox_from_keypoint
    ...
    def __call__(self, results):
        # img = drawKpts(results['img'], results['gt_keypointss'])
        # img = drawBoxes(img, results['gt_bboxes'])
        # import cv2
        # cv2.imwrite('test.jpg', img)
        # dict to albumentations format
        kpts_ = results['gt_keypointss']
        ...
        if 'keypoints' in results:
            ...
            try:
                results['keypoints'] = results['keypoints'].reshape(-1, 4, 3)
            except:
                print(results['keypoints'].shape)
                import cv2
                ...
                cv2.imwrite('test1.jpg', results['image'])
                assert False
        ...
        if self.refine_bbox_from_keypoint:
            new_bboxes = self.kpts2bboxes(results['gt_keypointss'])
            assert new_bboxes.shape[0] == results['gt_bboxes'].shape[0]
    ...
    def kpts2bboxes(self, kptss):
        new_bboxes = []
        for kpts in kptss:
            new_bbox = [kpts[:, 0].min(), kpts[:, 1].min(), kpts[:, 0].max(), kpts[:, 1].max()]
            new_bboxes.append(new_bbox)
        return np.array(new_bboxes, dtype=np.float16)
