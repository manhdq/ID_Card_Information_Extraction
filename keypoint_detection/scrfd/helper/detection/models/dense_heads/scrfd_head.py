...
from ....general.cnn import ConvModule, Scale, bias_init_with_prob, normal_init, DepthwiseSeparableConvModule
from ....general.cnn import constant_init, kaiming_init
from ....general.runner import force_fp32

from ...core import (anchor_inside_flags, bbox2distance, bbox_overlaps,
                    build_assigner, build_sampler, distance2bbox, distance2kps, kps2distance,
                    images_to_levels, multi_apply, multiclass_nms, multiclass_bboxes_kps_nms,
                    reduce_mean, unmap, )
...
class SCRFDHead(AnchorHead):
    ...
    def get_bboxes_kpss(self,
        ...
        """
        assert len(cls_scores) == len(bbox_preds) == len(kps_preds)
        ...
        for img_id in range(len(img_metas)):
            ...
            if with_nms:
                ...
                proposals = self._get_bboxes_kps_single(cls_score_list,
                                                    bbox_pred_list, kps_pred_list,
    ...
    def _get_bboxes_kps_single(self,
                           cls_scores,
                           bbox_preds,
                           kps_preds,
                           mlvl_anchors,
        ...
        assert len(cls_scores) == len(bbox_preds) == len(kps_preds) == len(mlvl_anchors)
        mlvl_bboxes = []
        mlvl_kps = []
        mlvl_scores = []
        for cls_score, bbox_pred, kps_pred, stride, anchors in zip(
            ...
            kps_pred = kps_pred.reshape( (-1,8) ) * stride[0]
            ...
            kps = distance2kps(
                self.anchor_center(anchors), kps_pred, max_shape=img_shape)
            mlvl_kps.append(kps)
        ...
        if rescale:
            ...
            mlvl_kps /= mlvl_kps.new_tensor(np.concatenate((scale_factor, scale_factor)))
        ...
        if with_nms:
            det_bboxes, det_kps, det_labels = multiclass_bboxes_kps_nms(mlvl_bboxes, mlvl_kps, mlvl_scores,
