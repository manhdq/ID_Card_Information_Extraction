from ...core import bbox2result, bboxkpss2result
...
class SCRFD(SingleStageDetector):
    ...
    def simple_test_end(self, img, img_metas, rescale=False):
        """Test function without test time augmentation.
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            #return outs
        if self.bbox_head.use_kps:
            bbox_kps_list = self.bbox_head.get_bboxes_kpss(
                *outs, img_metas, rescale=rescale)

            results = [
                bboxkpss2result(det_bboxes, det_kpss, det_labels, self.bbox_head.num_classes)
                for det_bboxes, det_kpss, det_labels in bbox_kps_list
            ]
            return results
        else:
            bbox_list = self.bbox_head.get_bboxes(
