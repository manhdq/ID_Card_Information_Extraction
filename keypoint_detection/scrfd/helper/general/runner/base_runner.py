...
from ..utils import mkdir_or_exist, Config
...
from .hooks import LoggerHook, TextLoggerHook
from .hooks import (CosineAnnealingLrUpdaterHook,
                    ...
                    StepLrUpdaterHook)
from .hooks import (CosineAnnealingMomentumUpdaterHook,
                    CyclicMomentumUpdaterHook,
                    LinearAnnealingMomentumUpdaterHook,
                    MomentumUpdaterHook,
                    OneCycleMomentumUpdaterHook,
                    StepMomentumUpdaterHook)
from .hooks import (Fp16OptimizerHook, GradientCumulativeFp16OptimizerHook,
...
class BaseRunner(metaclass=ABCMeta):
    ...
    def register_momentum_hook(
        ...
        if isinstance(momentum_config, dict):
            ...
            hook = eval(hook_type)(**momentum_config)
    ...
    def register_optimizer_hook(
        ...
        if isinstance(optimizer_config, dict):
            ...
            cfg = optimizer_config.copy()
            cfg_type = cfg.pop('type')
            hook = eval(cfg_type)(**cfg)
        else:
    ...
    def register_logger_hooks(self, log_config: Optional[Dict]) -> None:
        ...
        for info in log_config['hooks']:
            cfg = info.copy()
            cfg_type = cfg.pop('type')
            cfg = cfg | dict(interval=log_interval)
            logger_hook = eval(cfg_type)(**cfg)
